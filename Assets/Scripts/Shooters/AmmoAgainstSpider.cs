...
public class AmmoAgainstSpider : MonoBehaviour
{
    void Start()
    {
    ...
    public void OnTriggerEnter(Collider coll)
    {
        ...
        {
            if (coll.gameObject.TryGetComponent(out ShooterVsSpiderAgent target))
            {
                ...
                if(target.Ammo < target.initialAmmo)
                {
                    ...
                    transform.position = new Vector3(Random.Range(-4f, -4f), 0.5f, Random.Range(-4f, 4f)) + transform.parent.transform.position;
                }
            }
        }
    }
